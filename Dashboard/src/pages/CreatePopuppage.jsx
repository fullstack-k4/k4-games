...
import { createPopUp } from "@/store/Slices/popupSlice";
...
const CreatePopuppage = () => {
  ...
  const loading = useSelector((state) => state.popup.loading);
  ...
  const onSubmit = async (data) => {
    ...
    const response = await dispatch(createPopUp(data));
      ...
      <Container className="flex justify-center items-center min-h-screen">
        <div className="w-full max-w-lg p-6 bg-white dark:bg-gray-900 shadow-xl rounded-lg space-y-6 relative">
          ...
          <div className="absolute top-4 left-4">
            <Link to="/popups" className="flex items-center text-blue-600 dark:text-blue-400 hover:underline">
              <ArrowLeft className="w-5 h-5 mr-1" />
              Popups Page
            </Link>
          </div>

          <h2 className="text-2xl font-bold text-center">Add New Popup</h2>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            ...
            <div>
              ...
              <Input
                {...register("link", { required: "Popup link is required" })}
